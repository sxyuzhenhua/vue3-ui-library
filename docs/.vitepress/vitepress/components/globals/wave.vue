<template>
  <div>
    ...
    />
    <canvas ref="canvas" v-bind="$attrs" class="wave" />
  </div>
</template>
...
const bgColor = ref(rootStyle.getPropertyValue("--yu-color-primary"));
function animateWave() {
  if (!isClient) return;
  bgColor.value = hexToRgb(
    rootStyle.getPropertyValue("--yu-color-primary"),
    0.5
  );
  // 获取画布
